public class Combat extends Aircraft {
    ...
    public void atack(Airport airport) {
        ...
        for (Aircraft aircraft : airport.getAirport()) {
            if (aircraft instanceof Combat) {
                ...
                if (aircraft.isDestroyed()) continue;
                Coordinates cordsEnemy = aircraft.getCoordinates();
                if (Math.abs(cords.getX() - cordsEnemy.getX()) <= this.shotDistance && Math.abs(cords.getY() - cordsEnemy.getY()) <= this.shotDistance) {
